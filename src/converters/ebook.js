import { dlBlob, readAB, getExt } from '../utils.js';
export async function convEbook(file, target) {
  ...
  if (ext==='epub') {
    ...
    if (target==='txt') {
      ...
      dlBlob(new Blob([texts.join('\n\n')],{type:'text/plain;charset=utf-8'}), file.name.replace(/\.epub$/i,'.txt'));
      return;
    }
    if (target==='html') {
      let htmls=[];
      ...
      dlBlob(new Blob([combined],{type:'text/html;charset=utf-8'}), file.name.replace(/\.epub$/i,'.html'));
      return;
    }
  }
  if (['mobi','azw3'].includes(ext) && target==='txt') {
    ...
    dlBlob(new Blob([text],{type:'text/plain;charset=utf-8'}), file.name.replace(/\.[^.]+$/i,'.txt'));
    return;
  }
