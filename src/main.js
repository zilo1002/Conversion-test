...
function getSupportedTargets(catId, filename) {
    ...
    return '<div class="slide-in rounded-xl bg-slate-800/40 border border-slate-700/40 p-4 '+(item.status==='done'?'border-emerald-500/20':'')+'">'+
      ...
      '<div class="flex items-center gap-2">'+
        ...
        '(() => {
          ...
          const allTargets = cat.targets;
          if (!supported.length) {
            return '<span class="flex-1 px-3 py-1.5 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 text-sm">暂不支持转换此格式</span>';
          }
          let opts = '';
          for (const [v, info] of Object.entries(allTargets)) {
            if (supported.includes(v)) {
              opts += '<option value="' + v + '"' + (item.target === v ? ' selected' : '') + '>' + info.label + ' (.' + info.ext + ')</option>';
            }
          }
          return '<select id="t-' + idx + '" onchange="window.chgTarget(' + idx + ',this.value)" class="flex-1 bg-slate-800 border border-slate-600 rounded-lg px-3 py-1.5 text-sm focus:border-' + cat.color + '-500 focus:outline-none text-slate-200 pr-8">' + opts + '</select>';
        })()'+
      '</div>'+
...
function getMatrixHtml(catId) {
  const cat = CATEGORIES[catId];
  const matrix = {
    document: [
      { from: 'DOCX', to: ['TXT 纯文本', 'Markdown', 'HTML 网页', 'PDF 文档'], icon: '📝' },
      { from: 'XLSX / XLS / CSV / ODS', to: ['CSV 表格', 'JSON 数据', 'HTML 网页', 'XLSX 表格'], icon: '📊' },
      { from: 'TXT / Markdown', to: ['PDF 文档', 'HTML 网页'], icon: '📄' }
    ],
    image: [
      { from: 'JPG / JPEG', to: ['PNG', 'WebP', 'GIF', 'BMP'], icon: '🖼️' },
      { from: 'PNG', to: ['JPG', 'WebP', 'GIF', 'BMP'], icon: '🖼️' },
      { from: 'WebP', to: ['JPG', 'PNG', 'GIF', 'BMP'], icon: '🖼️' },
      { from: 'GIF', to: ['JPG', 'PNG', 'WebP', 'BMP'], icon: '🎞️' },
      { from: 'BMP', to: ['JPG', 'PNG', 'WebP', 'GIF'], icon: '🖼️' },
      { from: 'SVG 矢量图', to: ['PNG', 'JPG', 'WebP', 'BMP'], icon: '✏️' }
    ],
    ebook: [
      { from: 'EPUB', to: ['TXT 纯文本', 'HTML 网页'], icon: '📖' },
      { from: 'MOBI / AZW3', to: ['TXT 纯文本'], icon: '📱' }
    ],
    data: [
      { from: 'JSON', to: ['XML', 'CSV', 'YAML', 'TOML'], icon: '{ }' },
      { from: 'XML', to: ['JSON', 'CSV', 'YAML', 'TOML'], icon: '&lt;/&gt;' },
      { from: 'CSV', to: ['JSON', 'XML', 'YAML', 'TOML'], icon: '📋' },
      { from: 'YAML / YML', to: ['JSON', 'XML', 'CSV', 'TOML'], icon: '📄' },
      { from: 'TOML', to: ['JSON', 'XML', 'CSV', 'YAML'], icon: '⚙️' },
      { from: 'ZIP', to: ['提取文件内容', '重新打包'], icon: '📦' }
    ]
  };

  const items = matrix[catId] || [];
  return items.map(item => {
    const targets = item.to.map(t => '<span class="px-2 py-0.5 rounded bg-slate-700/50 text-slate-300 text-[10px] border border-slate-600/30">'+t+'</span>').join('');
    return '<div class="flex items-start gap-3 p-3 rounded-xl bg-slate-900/30 border border-slate-700/20">'+
      '<div class="w-8 h-8 rounded-lg bg-'+cat.color+'-500/10 '+cat.textColor+' flex items-center justify-center text-sm shrink-0 mt-0.5">'+item.icon+'</div>'+
      '<div class="flex-1 min-w-0">'+
        '<p class="text-xs font-medium text-slate-300 mb-1.5">'+item.from+'</p>'+
        '<div class="flex flex-wrap gap-1">'+targets+'</div>'+
      '</div>'+
      '<div class="text-slate-600 shrink-0 self-center">'+
